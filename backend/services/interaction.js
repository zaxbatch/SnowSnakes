...
class Interaction {
  static async toggleLike(userId, contentType, contentId) {
    const tableName = contentType + 's'; // e.g., 'comic' -> 'comics'
    ...
    if (like.rows.length > 0) {
      ...
      await pool.query(`UPDATE ${tableName} SET likes = likes - 1 WHERE id = $1`, [contentId]);
      return { liked: false };
    } else {
      // Like
      await pool.query(
  ...
  static async getComments(contentType, contentId) {
    try {
      ...
      console.warn(`Comments fetch failed for ${contentType}:`, err.message);
      return [];
    }
  }
