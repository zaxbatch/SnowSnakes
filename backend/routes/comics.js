...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (req.user) comic.isLiked = await Interaction.getLikeStatus(req.user.id, 'comic', req.params.id);
    res.json(comic);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const comic = await Comic.create({ title, scene, dialogue, caption, characters, image_url, author_id: req.user.id });
