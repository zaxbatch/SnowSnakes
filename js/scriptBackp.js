...
// ─── Simplified Snake, Match, Clicker (placeholders) ───
...
// ============================================
// 🎨 RENDER FUNCTIONS (with detailed logging)
// ============================================
...
function renderJokes() {
    ...
    console.log('📋 renderJokes: container =', container);
    if (!container) {
    ...
    console.log('📋 renderJokes: total jokes =', jokes.length);
    if (search) {
        ...
        console.log('📋 renderJokes: filtered jokes =', jokes.length);
    }
    ...
    if (jokes.length === 0) {
        ...
        console.log('📋 renderJokes: no jokes to render');
        return;
    }
    var html = '';
    ...
    console.log('📋 renderJokes: rendered ' + jokes.length + ' jokes');
...
function renderDoodles() {
    ...
    console.log('🎨 renderDoodles: container =', container);
    if (!container) {
        console.warn('⚠️ renderDoodles: container "doodle-list" not found!');
        return;
    }
    var doodles = Store.doodles;
    console.log('🎨 renderDoodles: total doodles =', doodles.length);
    if (doodles.length === 0) {
    ...
    container.innerHTML = html;
    console.log('🎨 renderDoodles: rendered ' + doodles.length + ' doodles');
...
function renderComics() {
    ...
    console.log('📢 renderComics: container =', container);
    if (!container) {
        console.warn('⚠️ renderComics: container "comic-list" not found!');
        return;
    }
    var comics = Store.comics;
    console.log('📢 renderComics: total comics =', comics.length);
    if (comics.length === 0) {
    ...
    container.innerHTML = html;
    console.log('📢 renderComics: rendered ' + comics.length + ' comics');
...
function renderSpread() {
    ...
    console.log('📺 renderSpread: container =', container);
    if (!container) {
        console.warn('⚠️ renderSpread: container "spread-list" not found!');
        return;
    }
    var episodes = Store.episodes;
    console.log('📺 renderSpread: total episodes =', episodes.length);
    ...
    container.innerHTML = html;
    console.log('📺 renderSpread: rendered ' + episodes.length + ' episodes');
}
...
function toggleKillerMode() {
    ...
    if (btn) {
        if (Store.killerMode) {
            ...
            document.querySelector('.app').style.borderColor = '#ff0000';
            document.querySelector('.app').style.borderStyle = 'double';
            ...
            document.querySelector('.app').style.borderColor = '#ff00ff';
            document.querySelector('.app').style.borderStyle = 'ridge';
        }
    }
}
...
function triggerEasterEgg() {
    ...
    if (easterClickCount === 3) {
        ...
        document.querySelector('.app').style.boxShadow = '0 0 50px rgba(255, 0, 255, 0.5)';
        easterClickCount = 0;
    }
}
...
function switchTab(tabName) {
    ...
    renderAll();
    if (tabName === 'spread') setTimeout(renderSpread, 50);
    if (tabName === 'games') setTimeout(function() { showGame('builtin'); }, 50);
}
...
function attachEventListeners() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (target) { e.preventDefault(); showGame('gallery'); }
    });
}
...
(function expose() {
    var fns = [
        'switchTab', 'showGame', 'renderSpread', 'renderAll', 'openModal', 'closeModal',
        'submitJoke', 'submitDoodle', 'submitComic', 'submitGame', 'submitLogin', 'logout',
        'getRandom', 'triggerEasterEgg', 'filterJokes', 'sortJokes', 'toggleLike', 'shareJoke',
        'voteGame', 'deleteGame', 'killJoke', 'playBuiltinGame', 'playUserGame',
        'moveToFridge', 'removeFromFridge', 'useUpCharacter', 'dragStart', 'dragEnd',
        'removeFile', 'toggleKillerMode'
    ];
